...
app.put("/addStop", async (req, res) => {
	try {
		const { offset, stopName, stopId, line, towerId } = req.body;
		if (offset === undefined || stopId === undefined || !line?.id) {
			return res.status(400).json({ error: 'Missing required fields: offset, stopId, and line.id are required.' });
		}

		const departureOffset = Number(offset);
		if (!Number.isFinite(departureOffset) || departureOffset > 0) {
			return res.status(400).json({ error: 'departureOffset must be a non-positive number.' });
		}
		...
		if (towerId) {
			const result = await pgClient.query(
				`INSERT INTO assignments (tower_id, stop_id, line_id, departure_offset)
				 VALUES ($1, $2, $3, $4)
				 RETURNING *`,
				[towerId, stopId, line.id, departureOffset]
			);
			return res.status(201).json({ status: 'created', assignment: result.rows[0] });
		}

		return res.status(200).json({
			status: 'accepted',
			note: 'Assignment request received. Add a towerId to persist assignments in the database.',
			payload: { stopName, stopId, line, departureOffset }
		});
	} catch (err) {
		console.error(err);
		res.status(500).json({ error: 'Unable to add assignment.' });
	}
});
